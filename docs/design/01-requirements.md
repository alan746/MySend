...
| FR-28 | Clipboard content, filenames, stored files, room authorizations, room metadata, and the reusable code reservation shall become eligible for physical removal immediately after logical closure. Cleanup runs every 15 minutes by default in bounded batches. | The next successful cleanup pass removes closed content while preserving active rooms. A failed storage deletion keeps a retryable record and does not falsely release the code or block later rooms. |
